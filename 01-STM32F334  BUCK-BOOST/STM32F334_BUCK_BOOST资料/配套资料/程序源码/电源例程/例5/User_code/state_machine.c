...
CCMRAM __IO uint16_t   Target_voltage  =12000;
CCMRAM __IO uint16_t   Target_current  =3000;        //目标电流
...
#define  VIN_OVP_POINT    			        36000                //输入过压保护点      单位（mV）
#define  VIN_OVP_RECOVERY_POINT    			34000                //输入过压保护恢复点  单位（mV）

#define  VIN_UVP_POINT    			        14000                //输入欠压保护点      单位（mV）
#define  VIN_UVP_RECOVERY_POINT    			16000                //输入欠压保护恢复点  单位（mV）

#define  VOUT_OVP_POINT    			       (Target_voltage+3000) //输出欠压保护点      单位（mV）
#define  VOUT_UVP_POINT    			       (Target_voltage-7000) //输出欠压保护点      单位（mV）
 
#define  IOUT_OCP_POINT                 5000                 //输出过流保护点      单位（mA）
#define  IIN_OCP_POINT                  8000                 //输出过流保护点      单位（mA）

#define  INPUT_POWER                    1000                  //最大输出功率        单位（W）
#define  OTP_POINT                      70                   //触发过温保护        单位（℃）
#define  OTP_RECOVERY_POINT             60                   //过温保护恢复点      单位（℃）
  ...
  HRTIM1->HRTIM_TIMERx[HRTIM_TIMERINDEX_TIMER_B].CMP1xR = 50;                          //更新HRTIMB（比较寄存器的值）
	HRTIM1->HRTIM_TIMERx[HRTIM_TIMERINDEX_TIMER_B].CMP3xR = 50;
	 ...
	 for(i=0;i<4;i++)
		...
		V_temp=0x00;
		Target_voltage=12000;
		Target_current  =3000;        //目标电流
...
CCMRAM void CS_DISPALY(void)
{
	...
	else if(Iout[3]<6000){LED_1A0_ON;LED_2A0_ON;LED_3A0_ON;LED_4A0_ON;LED_5A0_ON;}       //输出电流小于6.0A
	else if(Iout[3]<7000){LED_1A0_ON;LED_2A0_OFF;LED_3A0_OFF;LED_4A0_OFF;LED_5A0_ON;}    //输出电流小于7.0A
	else if(Iout[3]<8000){LED_1A0_OFF;LED_2A0_ON;LED_3A0_OFF;LED_4A0_OFF;LED_5A0_ON;}    //输出电流小于8.0A
	else if(Iout[3]<9000){LED_1A0_OFF;LED_2A0_OFF;LED_3A0_ON;LED_4A0_OFF;LED_5A0_ON;}    //输出电流小于9.0A
