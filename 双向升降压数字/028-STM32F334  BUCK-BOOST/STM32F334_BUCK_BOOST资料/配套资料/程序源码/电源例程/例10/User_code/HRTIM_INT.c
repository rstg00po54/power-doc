...
void TB1_L_TB2_H(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
 
  GPIO_InitStructure.GPIO_Mode  =  GPIO_Mode_OUT;     //选定为输出
  GPIO_InitStructure.GPIO_Pin   =  GPIO_Pin_10|GPIO_Pin_11;  //选定IO口
  GPIO_Init(GPIOA, &GPIO_InitStructure);  //初始化
	
	GPIOA->BRR  = GPIO_Pin_10;    //TB1输出低电平
	GPIOA->BSRR = GPIO_Pin_11;    //TB2输出高电平
	
	GPIO_InitStructure.GPIO_Pin   =  GPIO_Pin_8|GPIO_Pin_9;  //选定IO口
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
	GPIO_Init(GPIOA, &GPIO_InitStructure);  //初始化
...
void TA1_H_TA2_L(void)
{
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);  //初始化
	...
	GPIOA->BSRR = GPIO_Pin_8;    //TA1输出高电平
	GPIOA->BRR  = GPIO_Pin_9;    //TA2输出低电平
...
void HRTIM_Config(void)
{
  ...
  HRTIM_BaseInitStructure.RepetitionCounter = 2;                                //多少个周期触发一次中断
  ...
  HRTIM_TIM_OutputStructure.ResetSource = HRTIM_OUTPUTRESET_TIMCMP1|HRTIM_OUTPUTRESET_TIMCMP1|HRTIM_OUTPUTRESET_EEV_6;                   //计数到CMP1寄存器值，输出拉低
  ...
  HRTIM_DeadTimeConfig(HRTIM1, HRTIM_TIMERINDEX_TIMER_A, &HRTIM_TIM_DeadTimeStructure);            //开始配置定时器A
	...
	HRTIM_TIM_DeadTimeStructure.FallingValue = ((uint16_t)200);//下降沿死区时间
	HRTIM_TIM_DeadTimeStructure.RisingValue = ((uint16_t)200);//上升沿死区时间
	HRTIM_DeadTimeConfig(HRTIM1, HRTIM_TIMERINDEX_TIMER_B, &HRTIM_TIM_DeadTimeStructure);
	
	HRTIM_TIM_EVENT_Structure.FastMode=HRTIM_EVENTFASTMODE_ENABLE;
	HRTIM_TIM_EVENT_Structure.Filter=HRTIM_EVENTFILTER_1;
	HRTIM_TIM_EVENT_Structure.Polarity=HRTIM_EVENTPOLARITY_HIGH;
	HRTIM_TIM_EVENT_Structure.Sensitivity=HRTIM_EVENTSENSITIVITY_LEVEL;
	HRTIM_TIM_EVENT_Structure.Source=HRTIM_EVENTSRC_2;
	HRTIM_EventConfig(HRTIM1, HRTIM_EVENT_6,&HRTIM_TIM_EVENT_Structure);
	
	HRTIM_TimerEventFiltering_Structure.Filter=HRTIM_TIMEVENTFILTER_NONE;
	HRTIM_TimerEventFiltering_Structure.Latch=HRTIM_TIMEVENTLATCH_DISABLED;
	HRTIM_TimerEventFilteringConfig(HRTIM1,HRTIM_TIMERINDEX_TIMER_A,HRTIM_EVENT_6,&HRTIM_TimerEventFiltering_Structure);
  ...
  HRTIM_WaveformCompareConfig(HRTIM1, HRTIM_TIMERINDEX_TIMER_A, HRTIM_COMPAREUNIT_3, &HRTIM_CompareStructure);//设定为比较器3触发ADC
	
	HRTIM_ADCTrigStructure.Trigger      = HRTIM_ADCTRIGGEREVENT13_TIMERA_CMP3;                       //选择触发源
  HRTIM_ADCTrigStructure.UpdateSource = HRTIM_ADCTRIGGERUPDATE_TIMER_A;                            //HRTIMA计数回零时可以更新ADC触发来源
...
//非逐周期环路计算时可以采用以下中断，比如设置为2个PWM周期中断一次
  ...
  /* Start HRTIM's TIMER A */
	...
	HRTIM1->HRTIM_TIMERx[HRTIM_TIMERINDEX_TIMER_A].CMP1xR = 100;                                     //更新HRTIMA（比较寄存器的值）
  HRTIM1->HRTIM_TIMERx[HRTIM_TIMERINDEX_TIMER_A].CMP3xR = 50;
 
  HRTIM1->HRTIM_TIMERx[HRTIM_TIMERINDEX_TIMER_B].CMP1xR = 50;                          //更新HRTIMB（比较寄存器的值）
	HRTIM1->HRTIM_TIMERx[HRTIM_TIMERINDEX_TIMER_B].CMP3xR = 100;
